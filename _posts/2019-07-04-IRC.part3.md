...
![](/assets/images/irc-setup-part3/ZNC-network1.png)
...
1. Install `znc` on a server. The word 'server' is very important here. ZNC needs to be installed on a machine that is always running and always connected to the internet. Once the ZNC service has started, no changes should be made in `znc.conf`.
...
In my own setup, I use a VM (Compute Engie) running on Google Cloud Platform. Free credits can be obtained by applying for the [GitHub Student Developer Pack](https://education.github.com/pack). They provide AWS credits, Digital Ocean credits and much more.

Usually, it is advised that ZNC should operate over a seperate port, in which case a new firewall rule will has to be created in the network settings.
...
ZNC can be configured in two ways, using the `znc.conf` or using the `webadmin` interface. However, to perform the initial configuration ZNC has the `--makeconf` option. It will ask you to create a ZNC username and password, which will enable you to access the `webadmin` interface. The intial configuration process generates global, user, network and channel settings.
```bash
 ~ > znc --makeconf
...
<ul class="ul-circles">
	<li style="color: rgb(255,0,0);"><span style="color: #000;"><b>Timezone</b> - Set the appropriate timezone. </span></li>
	<li style="color: rgb(0,255,0);"><span style="color: #000;"><b>Auto Clear Chan Buffer</b> - This checks if channel buffers are automatically cleared after playback. When disabled, messages are buffered even while clients are attached, and already seen messages may be repeated each time clients connect. This is important when multiple clients are connected.</span></li>
	<li style="color: rgb(0,255,0);"><span style="color: #000;"><b>Auto Clear Query Buffer</b> -This checks if query buffers are automatically cleared after playback. When disabled, messages are buffered even while clients are attached, and already seen messages may be repeated each time clients connect. This is important when multiple clients are connected.</span></li>
	<li style="color: rgb(0,0,255);"><span style="color: #000;"><b>Buffer size</b> - Any arbitrary number is fine. </span></li>
	<li style="color: rgb(186,0,255);"><span style="color: #000;"><b>Add network</b></span></li>
</ul>
...
<frame>
<img src="/assets/images/irc-setup-part3/ZNC-webadmin-network-settings.png">
<div class="image-caption">
The network settings page.
</div>
</frame>

The first few sections are pretty straight-forward. The only puzzling part of network settings is where to put in the password for `nickserv`. This needs to be done using the nickserv module.
...
/server add -net freenode-znc -auto -ssl 35.244.43.39:5999 6697 tejas/freenode:password
/save
...
# Conclusions
There are still a lot of empty spaces, that will deal with various "modules", which will be covered in the next blog post.
